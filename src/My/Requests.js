...
function Requests()
{
  ...
  const navigate = useNavigate();
   ...
   useEffect(()=>{
      axios.get("http://52.53.149.31:3306/hotti").then(res=>setProducts(res.data));
  },[]);

  function Sh(e){
    const id = e.target.id;
    axios.get(`http://52.53.149.31:3306/rila/${id}`).then(
      res => setProducts(res.data));
    
    axios.get(`http://52.53.149.31:3306/tila/${id}`).then(
        res => setProducts(res.data));

  }
  function Show(e){
    const id = e.target.id;
    axios.get(`http://52.53.149.31:3306/merry/${id}`).then(
      res => setProducts(res.data));
    
    axios.get(`http://52.53.149.31:3306/sharry/${id}`).then(
        res => setProducts(res.data));
...
function Stop(e){
  ...
  if (window.confirm("Do you really want to leave?")) {
    var setName = window.prompt ('reason to reject the user' )
       ...
       axios.post("http://52.53.149.31:3306/yio",data).then(
        res=> console.log(res))
